...
const Training = () => {
    ...
    useEffect(() => {
        ...
        ws.onopen = () => {
            setCoachMessage("✅ Połączono z AI. Ustaw się w kadrze i kliknij SKALIBRUJ I START.");
        };
                ...
                if (isIdle) {
                    ...
                    if (phaseRef.current === 'ACTIVE') {
                        ...
                        if (hitPerfectFrameRef.current) {
                            ...
                            const summary = errs.length > 0 ? errs.join(" | ") : "Spróbuj jeszcze raz — skup się na dłoniach nad czołem i pracy nóg.";
                    ...
                    // FAZA: AKCJA (Ręce w górze)
                    if (phaseRef.current === 'IDLE') {
                        // Start nowego powtórzenia
                        setCoachMessage("👀 Analizuję technikę...");
                    }
                    phaseRef.current = 'ACTIVE';

                    // Zaliczenie repa tylko gdy backend potwierdzi szczyt (peak_valid):
                    // wysokie dłonie (front) + lekko ugięte kolana (bok).
                    if (msg.peak_valid) {
                        hitPerfectFrameRef.current = true;
                    } else {
                        // Zbieramy błędy do worka, na wypadek gdyby ruch całkowicie się nie udał
                        msg.issues.forEach(issue => {
                            if (issue.code !== 'idle' && issue.code !== 'low_visibility') {
        ...
        ws.onerror = () => {
            setCoachMessage("❌ Brak połączenia. Upewnij się, że w terminalu działa komenda 'uvicorn main:app'.");
        };
    ...
    useMediaPipe(videoFrontRef, canvasFrontRef, frontCameraId, (results) => {
        if (!isCalibrated || passType !== 'górne' || !results?.poseLandmarks) return;
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white flex flex-col p-4 md:p-6 font-sans">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <button onClick={() => navigate('/')} className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-xl font-bold shadow-lg">ZAKOŃCZ</button>
            </header>
                    ...
                    <div className="bg-black rounded-3xl relative overflow-hidden border border-blue-500/50 shadow-lg min-h-[400px]">
                        ...
                        {!isCalibrated && (
                            <div className="absolute inset-0 bg-black/60 z-20 flex items-center justify-center">
                                <p className="text-blue-400 font-bold mb-4">Ustaw się twarzą do kamery</p>
                                <button
                                    onClick={() => {
                                        setIsCalibrated(true);
                                        setCoachMessage("🎯 Kalibracja zakończona. Wykonaj pierwsze odbicie...");
                                    }}
